(()=>{
  ...
  #painel.sig-premium .sig-panel-hero{display:flex;justify-content:space-between;align-items:flex-start;gap:18px;margin-bottom:14px;padding:4px 4px 0}
  ...
  #painel.sig-premium .sig-metrics{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:10px;margin-bottom:16px}
  ...
  #painel.sig-premium .sig-metric-top{display:flex;align-items:center;gap:8px;min-width:0}
  #painel.sig-premium .sig-metric-icon{width:38px;height:38px;border-radius:12px;display:grid;place-items:center;font-size:18px;background:var(--soft,#edf5ff);transition:transform .16s ease;flex:0 0 auto}
  #painel.sig-premium .sig-metric:hover .sig-metric-icon{transform:scale(1.06)}
  #painel.sig-premium .sig-metric-label{font-family:Georgia,serif;font-weight:700;font-size:13px;line-height:1.15;color:#102c55;overflow-wrap:anywhere}
  #painel.sig-premium .sig-metric-value{font-family:Georgia,serif;font-weight:700;font-size:29px;line-height:1;margin:8px 0 2px 46px;color:#071b45}
  #painel.sig-premium .sig-metric-note{margin-left:46px;font-size:10.5px;color:#73849a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  ...
  #painel.sig-premium .sig-metric.rec{--accent:#e05d5d;--soft:#fff0f0}
  ...
  @media(max-width:1360px){#painel.sig-premium .sig-metrics{grid-template-columns:repeat(4,1fr)}}
  ...
  function diaSemana(data){try{return new Date(data).toLocaleDateString('pt-BR',{weekday:'long'})}catch(_){return ''}}
  function dataCurta(data){try{return new Date(data).toLocaleDateString('pt-BR')}catch(_){return ''}}
  function horaCurta(data){try{return new Date(data).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}catch(_){return ''}}
  function irPara(tela){const btn=document.querySelector(`#menu button[data-p="${tela}"]`);if(btn)btn.click()}

  function montar(){
    const p=document.getElementById('painel');
    const oabTopo=document.querySelector('.top .tag');if(oabTopo)oabTopo.style.display='none';
    if(!p||p.dataset.premium==='1')return;
    p.dataset.premium='1';p.classList.add('sig-premium');
    let s=document.getElementById('sigPainelPremiumStyle');if(!s){s=document.createElement('style');s.id='sigPainelPremiumStyle';document.head.appendChild(s)}s.textContent=css;
    p.innerHTML=`
      <div class="sig-panel-hero"><div><div class="sig-panel-kicker">Painel</div><h1 class="sig-panel-title">Painel</h1><div class="sig-panel-date" id="sigPainelData"></div></div><div class="sig-panel-brand"><h2>SIG - ADVOCACIA COLOMBO</h2><div class="sig-gold-line"></div><small>Gestão jurídica com propósito</small></div><div class="sig-oab-card">⚖ OAB/SP 510.497</div></div>
      <div class="sig-metrics">
        <div class="sig-metric proc" data-destino="processos" role="button" tabindex="0"><div class="sig-metric-top"><div class="sig-metric-icon">📁</div><div class="sig-metric-label">Processos ativos</div></div><div class="sig-metric-value" id="mproc">0</div><div class="sig-metric-note">Em andamento</div></div>
        ...
        <div class="sig-metric rec" data-destino="recorte" role="button" tabindex="0"><div class="sig-metric-top"><div class="sig-metric-icon">📰</div><div class="sig-metric-label">Recorte Digital</div></div><div class="sig-metric-value" id="mrec">0</div><div class="sig-metric-note">Publicações não analisadas</div></div>
      </div>
    ...
    p.querySelectorAll('.sig-metric[data-destino]').forEach(card=>{card.onclick=()=>irPara(card.dataset.destino);card.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();irPara(card.dataset.destino)}}});
  }

  function aprimorarResumo(){
    const box=document.getElementById('resumo');if(!box||typeof D==='undefined')return;
  ...
  async function atualizarAtendimentos(){
    const el=document.getElementById('mate');if(!el||typeof sb==='undefined')return;
    try{
  ...
  async function atualizarRecorte(){
    const el=document.getElementById('mrec');if(!el||typeof sb==='undefined')return;
    try{
  ...
  function atualizarNumeros(){
    ...
    if(e)e.textContent=(D.agenda||[]).filter(x=>x.inicio&&new Date(x.inicio)>=new Date()&&String(x.status||'').toLowerCase()!=='cancelado'&&String(x.status||'').toLowerCase()!=='arquivado').length;
